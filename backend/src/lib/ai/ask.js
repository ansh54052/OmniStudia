"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.BASE_SYSTEM_PROMPT = void 0;exports.askWithContext = askWithContext;exports.handleAsk = handleAsk;var _fs = _interopRequireDefault(require("fs"));
...
const BASE_SYSTEM_PROMPT = exports.BASE_SYSTEM_PROMPT = `
...
1. **Feynman Technique Integration**:
   ...
   - Use analogies that ilomnistudiate rather than obscure (bonus points for funny ones!)
...
const cacheDir = _path.default.join(process.cwd(), "storage", "cache", "ask");
if (!_fs.default.existsSync(cacheDir)) _fs.default.mkdirSync(cacheDir, { recursive: true });
const keyOf = (x) => _crypto.default.createHash("sha256").update(typeof x === "string" ? x : JSON.stringify(x)).digest("hex");
const readCache = (k) => {const f = _path.default.join(cacheDir, keyOf(k) + ".json");return _fs.default.existsSync(f) ? JSON.parse(_fs.default.readFileSync(f, "utf8")) : null;};
const writeCache = (k, v) => {const f = _path.default.join(cacheDir, keyOf(k) + ".json");_fs.default.writeFileSync(f, JSON.stringify(v));};
...
function serializeHistoryForCache(history) {
...
async function askWithContext(opts) {
  const rawQuestion = typeof opts.question === "string" ? opts.question : String(opts.question ?? "");
  const safeQ = (0, _normalize.normalizeTopic)(rawQuestion);
  ...
  const res = await _llm.default.call(messages);
...
historyArg)
{
  ...
  const safeQ = (0, _normalize.normalizeTopic)(questionRaw);
  const nsFinal = typeof ns === "string" && ns.trim() ? ns : "omnistudia";

  const rag = await (0, _runtime.execDirect)({
    agent: "researcher",
    plan: { steps: [{ tool: "rag.search", input: { q: safeQ, ns: nsFinal, k }, timeoutMs: 8000, retries: 1 }] },
    ctx: { ns: nsFinal }
  });

  const ctxDocs = Array.isArray(rag) ? rag : [];

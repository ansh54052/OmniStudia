"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.calculateUrgencyScore = calculateUrgencyScore;exports.generateSteps = generateSteps;exports.makeSlots = makeSlots;exports.parseTask = parseTask;exports.replan = replan;
...
notes: chapters 6-7`;
  ...
  try {
    const response = await (0, _ask.handleAsk)(systemPrompt + '\n\nText to parse: ' + text);
    ...
    return {
      ...
      priority: parsed.priority ? parseInt(parsed.priority) : fallback.priority,
...
function heuristicParse(text) {
  ...
  if (result.dueAt) {
    ...
    result.priority = Math.max(1, Math.min(5, Math.ceil((urgency + complexity) / 2)));
  }

  return result;
}

function parseDateHeuristic(dateStr) {
  const now = new Date();
...
Keep steps concise and actionable.`;
  ...
  try {
    const response = await (0, _ask.handleAsk)(systemPrompt + (task.notes ? '\nNotes: ' + task.notes : ''));
...
function makeSlots(tasks, policy) {
...
function generateTaskSlots(task, policy, startTime, existingSlots) {
  const slots = [];
...
function replan(missedSlots, remainingSlots, tasks, policy) {

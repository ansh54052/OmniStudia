"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.handleQuiz = handleQuiz;var _llm = _interopRequireDefault(require("../../utils/llm/llm"));
...
function stripFences(s) {return s.replace(/^\s*```(?:json)?\s*|\s*```\s*$/g, "").trim();}
function extractArray(s) {const m = s.match(/\[[\s\S]*\]/);return m ? m[0] : s;}
function tryParse(s) {try {return JSON.parse(s);} catch {return null;}}
function nstr(x, min = 1, max = 240) {const t = String(x ?? "").replace(/\s+/g, " ").trim();return t.length < min ? "" : t.slice(0, max);}
function to1_4(v) {
...
function splitOptions(v) {if (Array.isArray(v)) return v;if (typeof v === "string") return v.split(/[,;|]\s*/).map((s) => s.trim()).filter(Boolean);return [];}
...
function coerce(items) {
  ...
  const out = arr.slice(0, 5).map((o, i) => {
...
function validQuiz(a) {return Array.isArray(a) && a.length === 5 && a.every(validItem);}

async function ask(topicIn, sys) {
  const topic = (0, _normalize.normalizeTopic)(topicIn);
  const msgs = [
...
async function handleQuiz(topic) {

...
import { chatJSON, getChatDetail, createFlashcard, listFlashcards, deleteFlashcard, getChats, podcastStart } from "../lib/api";
...
function deriveTopicFromMarkdown(md) {
...
export default function Chat() {
  ...
  const [connecting, setConnecting] = useState(!!(initialChatId || initialQuestion));
  ...
  const keyFor = (kind, title, content) =>
  ...
  useEffect(() => {
    ...
    if (state.answer) {
      ...
      if (init.topic) setTopic(init.topic || "");else
      if (init.md) setTopic(deriveTopicFromMarkdown(init.md));
    } else {
      if (initialQuestion) {
    ...
    then((res) => {
      if (res?.ok && Array.isArray(res.messages)) {
        ...
        for (let i = normalized.length - 1; i >= 0; i--) {
          const raw = res.messages[i]?.content;
          if (normalized[i].role === "assistant") {
            const n = normalizePayload(raw);
            if (n.flashcards.length) setCards(n.flashcards);
            if (n.topic) setTopic(n.topic);else
            if (n.md) setTopic(deriveTopicFromMarkdown(n.md));
            break;
          }
        }
      }
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        if (m?.type === "answer") {
          ...
          if (norm.topic) setTopic(norm.topic);else
          if (norm.md) setTopic((t) => t || deriveTopicFromMarkdown(norm.md));
  ...
  const addToBag = async (kind, title, content) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await listFlashcards();
        const items = (res.flashcards || []).map((c) => ({
          id: c.id,

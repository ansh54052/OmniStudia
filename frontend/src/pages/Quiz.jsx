...
import { quizStart, connectQuizStream } from "../lib/api";
...
function takeQuizArray(a) {
  if (Array.isArray(a)) return a;
  if (Array.isArray(a?.quiz)) return a.quiz;
  return [];
}
...
export default function Quiz() {
  ...
  async function start(t) {
    ...
    try {
      ...
      const { close } = connectQuizStream(s.quizId, (ev) => {
        if (ev.type === "quiz") {
  ...
  const onSelect = (i) => {if (!showExp) setSelected(i);};
  ...
  const onNext = () => {
    ...
    setTimeout(() => {
      if (idx === total - 1) setDone(true);else
      {
        setIdx((n) => n + 1);
        setSelected(null);
        setShowHint(false);

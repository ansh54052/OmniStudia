...
import { connectPlannerStream, plannerDelete, plannerIngest, plannerList, plannerMaterials, plannerPlan, plannerUpdate, plannerWeekly, plannerCreateWithFiles, plannerUploadFiles, plannerDeleteFile } from "../../lib/api";
...
function DaySlots({ date, slots, tasks }) {
  return (
...
export default function Planner() {
  ...
  useEffect(() => {
    wsRef.current = connectPlannerStream(sid, (ev) => {
      if (ev.type === "plan.update") {
        setTasks((t) => t.map((x) => x.id === ev.taskId ? { ...x, plan: { ...x.plan, slots: ev.slots } } : x));
  ...
  const planTask = async (id) => {
    ...
    try {
      ...
      addNotification("error", "Failed to plan task: " + error?.message);
    } finally {
      setLoadingStates((prev) => ({ ...prev, [id]: { ...prev[id], plan: false } }));
    }
  };

  const gen = async (id, kind) => {
        ...
        <div className="grid gap-3">
          ...
          <div key={t.id} className="rounded-xl border border-zinc-800 bg-stone-950 p-3">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <select value={t.status} onChange={(e) => mark(t.id, e.target.value)} className="bg-stone-900 border border-zinc-800 text-stone-200 text-xs rounded px-2 py-1">
            ...
            <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                                        {materials[t.id].flashcards.flashcards.map((c, i) =>

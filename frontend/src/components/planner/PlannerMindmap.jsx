...
export default function PlannerMindmap({ tasks, plan, onPlan, onAssist, onUpdateStatus, onUpload, onDelete, onStartNow, onUpdateNotes }) {
  const wrapRef = useRef(null);
  const [positions, setPositions] = useState({});
  const [selected, setSelected] = useState(null);
  const [drag, setDrag] = useState(null);
  const [dragCustom, setDragCustom] = useState(null);
  ...
  const [stepPos, setStepPos] = useState({}); // key `${taskId}::${label}` in world coords
  ...
  function stepCountsFor(t, steps) {
    ...
    return Object.fromEntries(floored.map((x) => [x.label, x.v]));
  }
    ...
    (async () => {
      for (const t of tasks) {
        ...
        try {
          ...
          if (Array.isArray(data.importantTerms)) labels.push(...data.importantTerms.map((x) => x?.term).filter(Boolean));

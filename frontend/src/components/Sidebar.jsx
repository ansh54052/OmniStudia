...
export default function Sidebar() {
  ...
  const b = (t) => `p-2 rounded-xl duration-300 transition-all ${p === t ? 'text-white bg-white/10' : 'hover:bg-white/5 hover:text-white'}`;
  const chatButtonClass = `p-2 rounded-xl duration-300 transition-all ${isToggled ? 'text-white bg-white/10' : 'hover:bg-white/5 hover:text-white'}`;
  function toggleC() {
  ...
  return (
    <aside className='left-0 bottom-0 w-screen md:w-fit md:h-screen fixed p-4 z-50 lg:flex'>
      <div id='idk' className='w-64 md:order-2 h-full z-40 rounded-l-none rounded-2xl glass-panel hidden flex-col p-4 space-y-3 overflow-y-auto custom-scroll'>
        {chats?.chats.map((chat) =>
        <Link key={chat.id} to={`/chat/?chatId=${chat.id}`} className='p-2 hover:text-white hover:bg-white/10 w-full rounded-xl block text-sm min-h-fit truncate transition-colors'>
            {chat.title || 'Untitled Chat'}
          </Link>
        )}
      </div>
      <div id='idk0' className='md:w-20 md:order-1 h-full rounded-3xl glass-panel text-stone-300 flex flex-col items-center justify-between py-4'>
        <div className='flex flex-col items-center space-y-1 hidden md:flex'>
          <img src='/logo.png' alt='logo' className='w-10 h-auto rounded-full shadow-lg shadow-sky-500/20' />
          <span className='text-[10px] font-bold text-sky-400 tracking-wider uppercase'>OmniStudia</span>
        </div>
